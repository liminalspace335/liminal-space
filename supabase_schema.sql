...
create table if not exists public.branches (
  ...
  instagram text default '', facebook text default '', linktree text default '', tiktok text default '',
  sort int default 0
);
-- (이미 branches가 있는 경우 대비) 운영시간·링크트리·지점명 다국어 컬럼 보강
alter table public.branches add column if not exists linktree text default '';
alter table public.branches add column if not exists tiktok text default '';
